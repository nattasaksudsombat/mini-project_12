...
<div class="card mb-3">
    ...
    <div class="card-body">
        ...
        <div class="mb-3 border rounded p-3 d-none" id="new-address-wrapper">
            ...
            <div class="row g-2">
                <div class="col-md-3">
                    <label class="form-label">ชื่อที่อยู่ (เช่น บ้าน, ออฟฟิศ)</label>
                    <input type="text" name="new_address[name]" id="new-address-label"
                        class="form-control" placeholder="ระบุชื่อสถานที่">
                </div>
                <div class="col-md-9">
                    <label class="form-label">รายละเอียด (บ้านเลขที่ / ถนน / ซอย)</label>
                    <textarea name="new_address[address]" id="new-address-text"
                        class="form-control" rows="1" placeholder="บ้านเลขที่ หมู่ที่ ถนน ซอย..."></textarea>
                </div>

                {{-- ✅ เพิ่มให้ครบ 4 ช่อง: ตำบล, อำเภอ, จังหวัด, รหัสไปรษณีย์ --}}
                <div class="col-md-3">
                    <label class="form-label">ตำบล/แขวง</label>
                    <input type="text" name="new_address[subdistrict]" id="new-address-subdistrict"
                        class="form-control" required> {{-- เปลี่ยน name เป็น subdistrict --}}
                </div>
                <div class="col-md-3">
 ...
 */
// ค้นหา function prepareShippingAddressBeforeSubmit แล้วแก้ช่วงประกาศตัวแปร
function prepareShippingAddressBeforeSubmit() {
    ...
    const subdist = document.getElementById('new-address-subdistrict')?.value || ''; // ตำบล
    const dist = document.getElementById('new-address-district')?.value || '';       // อำเภอ
